...
router.post('/mediaRes', [
    validations_1.validaciones.jwt,
    express_validator_1.check('idIngreso', 'El ID del ingreso es un campo obligatorio').not().isEmpty(),
    express_validator_1.check('cantidad', 'La cantidad es un campo obligatorio').not().isEmpty(),
    validations_1.validaciones.campos
], ingreso_productos_controllers_1.IngresoProductoController.nuevaMediaRes);
// Nuevo cerdo
// POST - http://localhost:3000/api/ingreso_producto/cerdo
router.post('/cerdo', [
    validations_1.validaciones.jwt,
    express_validator_1.check('idIngreso', 'El ID del ingreso es un campo obligatorio').not().isEmpty(),
    express_validator_1.check('cantidad', 'La cantidad es un campo obligatorio').not().isEmpty(),
    validations_1.validaciones.campos
], ingreso_productos_controllers_1.IngresoProductoController.nuevoCerdo);
// Nuevo pollo
...
router.post('/pollo', [
    ...
    express_validator_1.check('cantidad', 'La cantidad es un campo obligatorio').not().isEmpty(),
    validations_1.validaciones.campos
], ingreso_productos_controllers_1.IngresoProductoController.nuevoPollo);
// Actualizar producto
